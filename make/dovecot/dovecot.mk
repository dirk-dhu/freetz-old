$(call PKG_INIT_BIN, 2.2.28)
$(PKG)_SOURCE:=dovecot-$($(PKG)_VERSION).tar.gz
$(PKG)_SOURCE_MD5:=fb46d90073252a0e3b7434988096206f
$(PKG)_SITE:=http://www.dovecot.org/releases/2.2

$(PKG)_STARTLEVEL=92 # after xmail

$(PKG)_BINARY:=$($(PKG)_DIR)/src/master/.libs/dovecot
$(PKG)_TARGET_BINARY:=$($(PKG)_DEST_DIR)/usr/sbin/dovecot
$(PKG)_TAR_CONFIG := $($(PKG)_DEST_DIR)/etc/default.dovecot/default_config/default_config.tar
...
$(PKG)_EXTRA_LDFLAGS := -Wl,--gc-sections -lcrypt -Wl,-rpath=/usr/lib/freetz
...
$(PKG)_CONFIGURE_OPTIONS += --with-storages=maildir
...
$(pkg)-precompiled: $($(PKG)_TARGET_BINARY) $($(PKG)_TAR_CONFIG)
