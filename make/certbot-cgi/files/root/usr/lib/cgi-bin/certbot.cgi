...
check "$CERTBOT_CHALLENGE" http-01:http tls-sni-01:https dns-01:dns 
...
cat << EOF
<p>
<input id="e3" type="radio" name="challenge" value="tls-sni-01"$https_chk><label for="e3">$(lang de:"TLS-SNI-01" en:"TLS-SNI-01")</label>
...
<p>$(lang de:"Wenn HTTP als bevorzugte Challenge genommen wird, wird ein http server auf dem Port auf den in Apache eingestellten HTTP Port oder 8081 gestartet, d.h. AVM-Forwarding vom Port 80 auf den Port 8095 muss vorhanden sein." 
          en:"Please remember, that certbot opens temporary a web service on the same port as used in Apache or port 8081, to verify the created certificate.
              In order for that, you have to create a port forwarding using the AVM Forwarding admin page from 80 to 8081!")<br>
